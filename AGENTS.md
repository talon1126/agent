...
- Warehouse inventory table provisioning and sync: `POST /warehouse/inventory-table/provision` creates a fixed-schema table inside an existing Feishu Bitable app/base; `POST /warehouse/inventory-table/sync` publishes a one-way snapshot from `mock-api /warehouse/inventory/{sku}`.
...
- Use `warehouse_inventory_table_provision_tool` only when users explicitly ask to create, initialize, or configure the Feishu inventory table. It creates a table in an existing Bitable app/base; it does not create the base or the inventory source of truth.
