...
- Do not add task status fields to the taskbook. Completion is derived from
  evidence under `artifacts/task-evidence/` and `artifacts/phase-evidence/`.
...
6. Run `python scripts/task_verify.py --task <TASK_ID>`. F5, G5, H5, and I5 also
   require `--quality-report <path>`.
7. A reviewer or CI runner independent from the implementing agent reruns the
   same verification. An agent's written claim is not completion evidence.
8. At a stage boundary run `python scripts/verify_phase_gate.py --milestone <ID>`.
...
A task is complete only when dependencies, frozen acceptance, path scope,
taskbook verification commands, applicable quality gates, evidence generation,
and independent rerun all pass. Preserve failure logs; do not represent partial
or mocked results as completion.
