...
- **设计RPA自动化采集**：搭建商品数据自动化采集与处理流水线，基于 **Playwright** 实现京东分类页商品 URL 自动发现、规范化与去重，使用 **影刀 RPA** 复用登录态完成商品详情采集并导出统一 CSV；通过 **pandas** 实现字段校验、数据清洗、价格标准化、失败数据隔离及批次对账，支持断点续跑、异常状态识别和标准化数据集输出。
