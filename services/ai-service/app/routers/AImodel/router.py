...
async def chat_with_aimodel(
    ...
    if len(await http_request.body()) > MAX_AIMODEL_REQUEST_BYTES:
        raise HTTPException(
            status_code=413,
            detail=f"AImodel request exceeds {MAX_AIMODEL_REQUEST_BYTES} bytes.",
        )
...
def list_aimodel_conversations(user_id: int) -> list[AiModelConversationSummary]:
    ...
    return [
        ...
        for conversation in get_aimodel_memory_store().list_conversations(
            user_id=user_id
        )
    ]
...
@router.get(
    ...
    response_model=list[AiModelStoredMessage],
)
def list_aimodel_conversation_messages(
