...
- `memory.py`：管理 `conversation`、`message`、`user_memory` 三张表，提供窗口记忆和长期偏好记忆。
...
- `user_id`：必填数字，首版前端复用现有 `CART_USER_ID = 1`，对应 `users.id`。后续接登录后再替换为真实登录用户 ID。
- `conversation_id`：可选数字 ID，对应后端 `conversation.id`。为空时后端创建新会话，并在 `done` 事件返回新 ID。
- `message`：必填，用户原始问题。
...
event: done
data: {"conversation_id":123,"answer":"agent 的完整自然语言回答","recommended_links":[]}
```
...
AImodel 首版使用 `ai-service` 管理三张表：

- `conversation`：保存当前聊天窗口会话。主键为 `id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY`，`title VARCHAR(40)`，不设置 `conversation_id` 文本字段。
- `message`：保存用户消息和 assistant 最终回答。主键为 `id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY`，`conversation_id INTEGER NOT NULL` 仅保存 `conversation.id`，不建立物理外键。
- `user_memory`：保存用户长期购物偏好，例如品牌偏好、价格偏好、类目偏好。
- 三张表的 `user_id` 均为 `INTEGER NOT NULL`，只改 AImodel 自建表，不改已有 `users` 表或其他业务表。
- 如果运行环境中已存在旧版字符串 `user_id` 数据，按用户确认的 B 方案清空 `conversation`、`message`、`user_memory` 后迁移字段类型。

窗口记忆规则：

- 每次请求根据 `conversation_id` 读取最近 5 条 `message`。
- 只注入 `role=user` 和 `role=assistant` 的自然语言正文。
- 不把工具调用 JSON、工具结果、隐藏推理或 system prompt 写入 `message`。
- 用户本轮消息会写入 `message.links`，assistant 最终回答会写入 `message.recommended_links`。

长期记忆规则：

- 长期记忆按前端传入的数字 `user_id` 归属。
- 首版使用规则型提取，不额外调用模型抽取偏好。
- 只保存明确表达的偏好，例如“喜欢小米”保存为 `brand_preference=小米`，“看重高性价比”保存为 `price_preference=高性价比`。
- 长期偏好会在后续请求中作为上下文注入 LangChain messages，但不会在 SSE `done` 中返回。
...
- `aiModelApi.ts`：封装 `POST /AImodel/chat` 的 SSE 流式读取，以及会话列表和历史消息查询。
...
- 对同一窗口的连续追问，结合最近 5 条历史 message 理解上下文。
- 对同一数字用户的后续推荐，结合长期偏好辅助排序和表达。
...
前端页面
  ...
  -> 打开 AImodel 对话面板
  -> 使用现有 CART_USER_ID=1
  -> 查询 /AImodel/conversations 供用户选择旧会话或新对话
  -> POST /AImodel/chat
  -> ai-service AImodel router
  -> 创建或确认 conversation.id
  -> 读取最近 5 条 message 和 user_memory 长期偏好
...
- `user_id` 为空：返回 422，由 Pydantic 校验处理。
- `DASHSCOPE_API_KEY` 缺失：返回 503。
- 数据库不可用且配置了 `DATABASE_URL`：通过 SSE `error` 事件返回，避免用户误以为记忆已经保存。
...
- `/AImodel/chat` 路由已挂载。
- 请求必须携带 `user_id`，`conversation_id` 为数字或空。
- `conversation_id` 为空时创建新 `conversation`，并在 `done` 返回数字 ID。
- `message` 只使用逻辑外键保存 `conversation.id`，不创建物理外键。
- 当前窗口只注入最近 5 条历史 message。
...
- 前端 `aiModelApi.ts` 会按约定提交 `user_id`、`conversation_id`、`message` 和 `links`，并解析 `status`、`delta`、`done` 事件。
- 前端使用现有 `CART_USER_ID = 1` 作为首版用户 ID。
- 前端打开 `AI模式` 时调用 `GET /AImodel/conversations?user_id=1` 查询全部会话；选择旧会话后调用 `GET /AImodel/conversations/{conversation_id}/messages?user_id=1` 加载历史消息。
