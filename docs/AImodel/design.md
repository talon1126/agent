...
- `user_id`：必填。前端从 `localStorage.aimodel_user_id` 读取；不存在时生成 `anon_` 开头的本地匿名 ID 并持久化。
...
- `aiModelApi.ts`：封装 `POST /AImodel/chat` 的 SSE 流式读取，并维护 `localStorage.aimodel_user_id`。
...
前端页面
  -> 点击侧边栏 AI模式
  -> 打开 AImodel 对话面板
  -> 读取或生成 localStorage.aimodel_user_id
  -> POST /AImodel/chat
...
- 前端首次请求会生成并复用 `localStorage.aimodel_user_id`。
